...
export function AdminDashboard() {
    ...
    return [
      {
        ...
        value: formatMoney(stats.finance.canonical.totalRevenueUsd),
        source: stats.kpiSource.builderRevenue,
      },
      {
        label: "Routed volume",
        value: formatCompact(stats.finance.canonical.totalVolumeUsd),
        source: stats.kpiSource.routedVolume,
      },
      {
        label: "Fills",
        value: String(stats.finance.canonical.fillsCount),
        source: stats.kpiSource.fills,
      },
      {
        label: "Active traders",
        value: String(stats.finance.canonical.totalUsers),
        source: stats.kpiSource.activeUsers,
      },
      {
        label: "Avg rev / user",
        value: formatMoney(stats.finance.canonical.avgRevenuePerUser),
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-5 text-foreground md:px-6">
      <div className="mx-auto flex max-w-[1450px] gap-4">
        <aside className="hidden w-[220px] shrink-0 rounded-[16px] border border-white/10 bg-[#090f1d]/85 p-3 lg:block">
              ...
              <button
                ...
                className={`flex w-full items-center justify-between rounded-[10px] px-3 py-2 text-left text-sm transition ${
                  active
                    ...
                    : "text-foreground/55 hover:bg-white/[0.06] hover:text-white/85"
                }`}
              >
                {label}
                {!active ? (
        ...
        <div className="min-w-0 flex-1">
          <div className="glass-card mb-4 flex flex-wrap items-center justify-between gap-3 px-4 py-3">
            ...
            <div className="relative min-w-[240px] flex-1 md:max-w-[380px]">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-white/35" />
              <input
                placeholder="Search users, wallets, sources..."
                className="h-10 w-full rounded-[12px] border border-white/10 bg-white/[0.03] pl-9 pr-3 text-sm text-white outline-none placeholder:text-white/35"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={loading}
                className="inline-flex h-10 items-center gap-2 rounded-[10px] border border-white/10 bg-white/[0.04] px-3 text-sm text-foreground/70 transition hover:bg-white/[0.08] hover:text-white disabled:opacity-50"
              >
                {loading ? (
          ...
          <section className="glass-card p-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h1 className="text-3xl font-semibold tracking-[-0.04em] text-white">Today</h1>
              <p className="text-xs text-foreground/40">
                Canonical sync: {stats?.canonicalSync.freshness ?? "unknown"} ·{" "}
                {stats ? timeAgo(stats.canonicalSync.lastSyncedAt) : "—"}
              </p>
            </div>
          ...
          <section className="glass-card mt-4 p-5">
              ...
              {statsCards.map((card) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {sourceBadge(card.source ?? "pipeline")}
                  </div>
            ...
            <div className="mt-4 rounded-[12px] border border-white/10 bg-white/[0.02] p-4">
          ...
          <section className="glass-card mt-4 p-5">
            ...
            <p className="mb-4 text-xs text-foreground/45">
              Canonical L1 totals vs pipeline estimates for the selected window.
            </p>
            <div className="grid gap-3 md:grid-cols-4">
              {[
                ["Revenue", rec?.revenue.canonical ?? 0, rec?.revenue.pipeline ?? 0, rec?.revenue.delta ?? 0, true],
                ["Volume", rec?.volume.canonical ?? 0, rec?.volume.pipeline ?? 0, rec?.volume.delta ?? 0, true],
                ["Fills", rec?.fills.canonical ?? 0, rec?.fills.pipeline ?? 0, rec?.fills.delta ?? 0, false],
                ["Users", rec?.users.canonical ?? 0, rec?.users.pipeline ?? 0, rec?.users.delta ?? 0, false],
              ].map(([label, canonical, pipeline, delta, money]) => (
                <div key={String(label)} className="rounded-[10px] border border-white/10 bg-white/[0.03] p-3">
                  ...
                  <p className="text-sm text-foreground/55">
                    P: {money ? formatMoney(Number(pipeline)) : Number(pipeline)}
                  </p>
          ...
          <section className="glass-card mt-4 p-5">
            ...
            <div className="max-h-[320px] overflow-auto rounded-[12px] border border-white/10">
